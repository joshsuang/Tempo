...
type Settings = {
  ...
  miniTimer: boolean
  showRecentCountdowns: boolean
  untilCompletionBehavior: 'stop' | 'countdown' | 'complete'
  keepAwake: boolean
  sound: boolean
  soundType: 'bell' | 'chime' | 'tone'
  volume: number
...
const initialSettings: Settings = {
  ...
  autoStartBreaks: true, autoStartFocus: false, showSeconds: true, confirmReset: true, clearTaskOnComplete: false, keepTaskAfterComplete: true, completionBehavior: 'choices', focusIntensity: 'focused', miniTimer: false, showRecentCountdowns: true, untilCompletionBehavior: 'complete', keepAwake: false,
  sound: true, soundType: 'bell', volume: 70, rememberLastMode: true, focusMode: true, notifications: true, lastMode: 'pomodoro',
}
...
const playCompletionSound = (volume: number, soundType: 'bell' | 'chime' | 'tone' = 'bell') => {
  try {
    ...
    oscillator.type = soundType === 'tone' ? 'sine' : 'triangle'; oscillator.frequency.value = soundType === 'bell' ? 660 : soundType === 'chime' ? 520 : 420
...
function App() {
  ...
  const syncTimer = useCallback((now = Date.now()) => {
    ...
    if (mode === 'countdown') {
      ...
      if (next <= 0) {
        if (settings.sound) playCompletionSound(settings.volume, settings.soundType)
    ...
    if (mode === 'until') {
      ...
      setRemaining((value) => {
        if (value > 0 && next <= 0) {
          ...
          recordSession(untilDuration); setCompletion(settings.completionBehavior === 'choices' || settings.untilCompletionBehavior === 'complete' ? { title: 'Until time reached', body: task || 'Your target time has arrived' } : null)
          if (settings.untilCompletionBehavior === 'countdown') { setMode('countdown'); setRemaining(5 * 60); setCountdownDuration(5 * 60); setEndAt(Date.now() + 5 * 60 * 1000); setRunning(true); return 0 }
          setRunning(false)
          setEndAt(null)
        }
        return next
  ...
  }, [running, mode, phase, endAt, stopwatchStartedAt, selectedPreset, settings.autoStartBreaks, settings.autoStartFocus, settings.sound, settings.soundType, settings.volume, settings.completionBehavior, settings.untilCompletionBehavior, settings.clearTaskOnComplete, getUntilSeconds, task, recordSession])
  ...
  function previewSound() { playCompletionSound(settings.volume, settings.soundType) }
...
function TimerPage(props: TimerPageProps) {
  ...
  return <section className="timer-page">
    ...
    <div className="timer-stage">
      ...
      {mode === 'countdown' && <div className="countdown-picker"><label htmlFor="countdown-minutes">COUNT DOWN FROM</label><div className="countdown-input"><input id="countdown-minutes" aria-label="Custom countdown duration" type="text" inputMode="text" placeholder="45m or 1h 20m" value={countdownInput} disabled={running} onBlur={() => { if (!parseDuration(countdownInput)) setCountdownInput(`${customCountdownMinutes}m`) }} onChange={(event) => { setCountdownInput(event.target.value); const parsed = parseDuration(event.target.value); if (parsed) setCountdownValue(parsed) }} /><span>minutes</span></div><small>Try 45m or 1h 20m</small><div className="recent-duration-list" aria-label="Countdown presets">{countdownQuickPresets.map((preset) => <button type="button" key={preset.name} onClick={() => setCustomCountdownMinutes(preset.minutes)}>{preset.name} · {preset.minutes}m</button>)}{settings.showRecentCountdowns && recentDurations.map((duration) => <button type="button" key={`recent-${duration}`} onClick={() => setCustomCountdownMinutes(duration)}>Recent · {duration}m</button>)}</div></div>}

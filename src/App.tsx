...
function TimerPage(props: TimerPageProps) {
  ...
  return <section className="timer-page">
    ...
    <div className="timer-footer">
      <div className="context-block"><span className="context-label">{mode === 'pomodoro' ? 'CURRENT PRESET' : mode === 'until' ? 'TARGET TIME' : 'SESSION'}</span>{mode === 'pomodoro' ? <div className="preset-select-wrap"><button className="preset-select" onClick={() => setShowPresets((value) => !value)}>{selectedPreset.name}<span>⌄</span></button>{showPresets && <div className="preset-menu">{presets.map((preset) => <button className={preset.id === selectedPresetId ? 'chosen' : ''} key={preset.id} onClick={() => { changePreset(preset.id); setShowPresets(false) }}>{preset.name}<span>{preset.focus}/{preset.break}</span></button>)}</div>}</div> : mode === 'until' ? <div className="until-input"><input aria-label="Target time" type="time" value={untilTarget} onChange={(event) => setUntilTarget(event.target.value)} /> <span>{getUntilSeconds() > 0 ? `${Math.ceil(getUntilSeconds() / 60)} min remaining` : 'Choose a future time'}</span></div> : <span className="footer-value">No time limit</span>}</div>
